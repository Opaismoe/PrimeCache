...
const mockBrowser = { newContext: mockNewContext } as unknown as Browser;

describe('createContext', () => {
  it('creates a new browser context', async () => {
    const { createContext } = await import('./context');
    const ctx = await createContext(mockBrowser);
  ...
  it('rotates locale across multiple calls', async () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      await createContext(mockBrowser);
      locales.add(mockNewContext.mock.calls.at(-1)?.[0].locale);
    }
  ...
  it('sets a custom userAgent when provided', async () => {
    const { createContext } = await import('./context');
    await createContext(mockBrowser, 'TerraCache/1.0.0');
    const options = mockNewContext.mock.calls.at(-1)?.[0];
