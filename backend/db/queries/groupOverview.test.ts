...
describe('getGroupOverview series', () => {
    ...
    await db.insert(visits).values([
      {
        ...
        url: 'https://a.com/',
        load_time_ms: 400,
        visited_at: new Date('2025-03-01T10:01:00Z'),
        error: null,
      },
      {
        run_id: run.id,
        url: 'https://a.com/p',
        load_time_ms: 500,
        visited_at: new Date('2025-03-01T10:02:00Z'),
        error: null,
      },
    ]);
    ...
    await db.insert(visits).values([
      {
        ...
        url: 'https://b.com/',
        load_time_ms: 400,
        visited_at: new Date('2025-03-02T10:01:00Z'),
        error: null,
      },
      {
        run_id: run.id,
        url: 'https://b.com/p',
        load_time_ms: 500,
        visited_at: new Date('2025-03-02T10:02:00Z'),
        error: 'timeout',
      },
    ]);
  ...
  it('series includes avgSeoScore when visit_seo data exists', async () => {
    ...
    await db.insert(visit_seo).values({
      ...
      meta_description:
        'A perfectly crafted meta description that is between fifty and one hundred sixty characters long here.', // 50-160 → 20pts
...
describe('getGroupOverview — cancelled run exclusion', () => {
  it('excludes cancelled run visits from stats/series but keeps them in recentRuns', async () => {
    const db = await createTestDb();
    const { getGroupOverview } = await import('./groupOverview');
    ...
    expect(result.stats.avgLoadTimeMs).toBe(400);
    expect(result.series).toHaveLength(1);
