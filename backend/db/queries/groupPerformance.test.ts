...
describe('getGroupPerformance — cancelled run exclusion', () => {
  it('excludes cancelled run visits from performance metrics', async () => {
    ...
    const [completed] = await db.insert(runs).values({
      group_name: 'perf-cancel', started_at: new Date('2025-01-01T10:00:00Z'), status: 'completed',
    }).returning()
    await db.insert(visits).values({
      run_id: completed.id, url: 'https://d.com/', load_time_ms: 500,
      visited_at: new Date('2025-01-01T10:01:00Z'), error: null,
    })

    const [cancelled] = await db.insert(runs).values({
      group_name: 'perf-cancel', started_at: new Date('2025-01-02T10:00:00Z'), status: 'cancelled',
    }).returning()
    await db.insert(visits).values({
      run_id: cancelled.id, url: 'https://d.com/', load_time_ms: 9999,
