...
export async function upsertSecret(db: Db, name: string, encryptedValue: string): Promise<SecretRow> {
  const [row] = await db
    .insert(secrets)
...
export async function listSecrets(db: Db): Promise<SecretListRow[]> {
  return db
    .select({ id: secrets.id, name: secrets.name, created_at: secrets.created_at, updated_at: secrets.updated_at })
    .from(secrets);
}
