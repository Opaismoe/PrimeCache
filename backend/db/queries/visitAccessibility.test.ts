...
describe('visitAccessibility queries', () => {
  it('insertVisitAccessibility and getAccessibilityByVisitId round-trip', async () => {
    ...
    const { insertVisitAccessibility, getAccessibilityByVisitId } = await import(
      './visitAccessibility'
    );
    // insert run + visit first
    const [run] = await db
      .insert(runs)
      .values({ group_name: 'a11y-test', started_at: new Date(), status: 'completed' })
      .returning();
  ...
  it('getGroupAccessibility returns per-URL summary', async () => {
    const db = await createTestDb();
    const { insertVisitAccessibility, getGroupAccessibility } = await import(
      './visitAccessibility'
    );
    const [run] = await db
      .insert(runs)
      .values({ group_name: 'a11y-group', started_at: new Date(), status: 'completed' })
      .returning();
    const [visit] = await db
      .insert(visits)
      .values({
