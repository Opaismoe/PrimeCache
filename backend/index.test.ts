...
vi.mock('./api/server', () => ({
  buildServer: vi.fn().mockResolvedValue({ listen: mockListen, close: mockClose, ready: vi.fn().mockResolvedValue(undefined) }),
}))
...
describe('boot sequence', () => {
  it('runs migrations before starting the API server', async () => {
    vi.resetModules()
    const order: string[] = []
    mockMigrate.mockImplementation(async () => { order.push('migrate') })
    mockListen.mockImplementation(async () => { order.push('listen') })
