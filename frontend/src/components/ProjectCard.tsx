  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div className="font-mono" style={{ fontSize: 12, color: 'var(--muted-foreground)', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {host}
            </div>
          </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, color: 'var(--muted-foreground)' }}>
        ...
        <span className="font-mono" style={{ flexShrink: 0 }}>{group.urls.length} URLs</span>
      </div>

      {/* Footer metrics */}
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11.5, color: 'var(--muted-foreground)', paddingTop: 10, borderTop: '1px solid var(--border)' }}>
        <span>
          Uptime <span className="font-mono" style={{ color: uptimeColor }}>{uptime !== null ? `${uptime.toFixed(1)}%` : '—'}</span>
        </span>
        <span>
          Avg <span className="font-mono" style={{ color: 'var(--foreground)' }}>{avgLoad !== null ? `${Math.round(avgLoad)}ms` : '—'}</span>
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {hasIssues && (
            <span style={{ background: 'var(--pc-warn-soft)', color: 'var(--pc-warn)', fontSize: 10, padding: '1px 7px', borderRadius: 999 }}>
              Needs work
            </span>
          )}
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <button
          ...
          style={{ flex: 1, height: 30, borderRadius: 7, fontSize: 12.5, background: 'var(--muted)', border: '1px solid var(--border)', color: 'var(--muted-foreground)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: isTriggering ? 'not-allowed' : 'pointer', opacity: isTriggering ? 0.6 : 1, transition: 'background 0.12s, color 0.12s' }}
        ...
        <Link
          ...
          style={{ flex: 1, height: 30, borderRadius: 7, fontSize: 12.5, background: 'var(--muted)', border: '1px solid var(--border)', color: 'var(--muted-foreground)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, textDecoration: 'none', transition: 'background 0.12s, color 0.12s' }}
          className="hover:bg-card hover:text-foreground"
        >
        ...
        <Link
          ...
          style={{ width: 30, height: 30, borderRadius: 7, background: 'var(--muted)', border: '1px solid var(--border)', color: 'var(--muted-foreground)', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', transition: 'background 0.12s, color 0.12s' }}
          className="hover:bg-card hover:text-foreground"
        >
